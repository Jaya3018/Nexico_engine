...
if mode.startswith("📁"):
    ...
    if uploaded_file is not None:
        try:
            pnl = pd.read_excel(uploaded_file, sheet_name="P&L")
            bs = pd.read_excel(uploaded_file, sheet_name="Balance Sheet")
            cf = pd.read_excel(uploaded_file, sheet_name="Cash Flow")
            ...
            st.error(f"Couldn't read the file — check sheet names match exactly. Error: {e}")
            st.stop()
    else:
...
def forecast_metric(pnl, column, growth_override, years_ahead=2):
    ...
    return pd.concat([hist, fcst], ignore_index=True), cagr
...
_, hist_cagr = forecast_metric(pnl, metric_choice, None)
growth_input = st.slider("Assumed forward growth rate (%)", -20.0, 40.0, round(hist_cagr * 100, 1), 0.5)
combined, _ = forecast_metric(pnl, metric_choice, growth_input / 100)
...
with col1:
    t_main = st.text_input("Company ticker", value=main_ticker or "")
with col2:
    t_peers = st.text_input("Peer tickers (comma-separated)", value=", ".join(peer_tickers_list))

if st.button("Fetch live market data") and t_main:
    ...
    for t in tickers:
        try:
            ...
            rows.append({
                "Ticker": t, "Price": info.get("currentPrice", "N/A"),
                "Market Cap (Cr)": round(info.get("marketCap", 0) / 1e7, 0) if info.get("marketCap") else "N/A",
                "P/E Ratio": info.get("trailingPE", "N/A"),
            })
        except Exception:
            rows.append({"Ticker": t, "Price": "Error", "Market Cap (Cr)": "-", "P/E Ratio": "-"})
